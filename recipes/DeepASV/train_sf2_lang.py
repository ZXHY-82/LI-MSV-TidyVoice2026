...
class LocalTrainer(Trainer):
    ...
    def compute_forward(self, inputs, stage):
        ...
        # spk_output, loss_output = self.modules['classifier'](input=emb_output, label=inputs['spk_labels'])
        # predictions = {'emb_output':emb_output, 'spk_output':spk_output, 'loss_output':loss_output}
        predictions = {'emb_output':emb_output,'lang_output':lang_output}
    ...
    def loss_fn(self, inputs, predictions, stage):
        ...
        loss = F.cross_entropy(input=predictions['lang_output'], target=inputs['lang_labels'])
    ...
    def eval_fn(self, inputs, predictions):   
        ...
        spk_true = inputs['lang_labels'].detach().cpu().tolist()
        spk_pred = predictions['lang_output'].argmax(dim=-1).detach().cpu().tolist()
        acc = accuracy_score(y_true=spk_true, y_pred=spk_pred)
