...
import torch
import numpy as np
from deeplab.utils.fileio import load_scp, load_audio
...
from deeplab.dataio.feature import signal2tensor
...
class Train_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams):
        ...
        langs = set()
        self.utt2lang = {}
        with open(hparams['train_utt2lang'], 'r') as f:
            for line in f.readlines():
                tmp = line.strip().split()
                self.utt2lang[tmp[0]] = tmp[1]
                langs.add(tmp[1])
        langs = sorted(list(langs))
        self.lang2label = {}
        for idx, lang in enumerate(langs):
            self.lang2label[lang] = idx

        
        utt_list = []
        for idx, spk_id in enumerate(self.spk_ids):
            for utt in self.spk2utt[spk_id]:
                utt_data = dict(wav_path=utt, spk_label=idx, speed_shift=None, lang=self.utt2lang[utt])
    ...
    def __getitem__(self, idx_data):
        ...
        lang_labels = torch.tensor(self.lang2label[utt_data['lang']]).long()
...
class Valid_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams):
        ...
        with open(hparams['test_utt2lang'], 'r') as f:
            for line in f.readlines():
                tmp = line.strip().split()
                self.utt2lang[tmp[0]] = tmp[1]
    ...
    def __getitem__(self, idx): 
        ...
        lang = self.utt2lang[scp_data['wav_path']]
        
        aud_inputs = torch.from_numpy(signal).float()
        utt_labels = torch.tensor(idx).long()
        lang_labels = torch.tensor(self.lang2label[lang]).long()
