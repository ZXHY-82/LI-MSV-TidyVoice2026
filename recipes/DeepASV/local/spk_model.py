...
class Audio2Vec_based_Adapter_Language(nn.Module):
    def __init__(
        self,
        model_name='openai/whisper-tiny', 
        frozen_encoder=True,
        ...
        dropout=0,
        spk_embd=False,
        frozen_spkmodel=True,
        GRL=False,
        grl_lambda=1.0,
        lang_num=40,
        ):
        super(Audio2Vec_based_Adapter_Language, self).__init__()
        ...
        self.adapter_layers = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.front.d_model, adapter_dim), # 1024
                nn.LayerNorm(adapter_dim),
                nn.ReLU(True),
                nn.Linear(adapter_dim, adapter_dim),
            ) for _ in range(self.n_mfa_layers)
        ...
        self.lang_head = nn.Sequential(
                ...
                nn.ReLU(True),
                nn.Linear(128, lang_num),
            )

        if frozen_spkmodel:
            self.freeze_model(self.adapter_layers)
            self.freeze_model(self.pooling)
            self.freeze_model(self.bottleneck)
            self.spkmodel_freeze = True
        else:
            self.spkmodel_freeze = False
        self.spk_embd = spk_embd
        self.GRL = GRL
    ...
    def forward(self, x):
        ...
        if self.spkmodel_freeze:
            ...
            x = self.bottleneck(x)
        if self.GRL:
            x_grl = GradReverse.apply(x, self.grl_lambda)
            x_lang = self.lang_head(x_grl)
        else:
            x_lang = self.lang_head(x)
        if self.spk_embd:
            return x, x_lang
        return x_lang
