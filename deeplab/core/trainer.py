    ...
    def load_checkpoints(self, ckpt_path):
        ...
        for key, module in self.modules.items():
            if key not in ckpt_data['modules']:
                ...
                if 'bias' in ckpt_data['modules'][key] and 'bias' in curr_state_dict:
                    curr_state_dict['bias'] = ckpt_data['modules'][key]['bias']
                if curr_len == ckpt_len:
                    curr_state_dict['weight'] = ckpt_data['modules'][key]['weight']
        ...
        log_path = os.path.join(os.path.dirname(ckpt_path), 'logs.json')
        if os.path.exists(log_path):
            self.init_logs_list = read_json(log_path)[:self.init_epoch_idx]
        else:
            self.init_logs_list = []
        if self.is_distributed:
            dist.barrier()
